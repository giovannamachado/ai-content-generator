...
const InstagramPost = ({ imagePrompt, caption, hashtags }: InstagramPostProps) => {
  ...
  // Ref para o scroll automático dos comentários
  ...
  // Atualiza o estado se as props mudarem
  useEffect(() => {
    setDisplayCaption(caption);
    setDisplayHashtags(hashtags);
  }, [caption, hashtags]);

  // Efeito para fazer scroll para o último comentário quando a lista cresce
  useEffect(() => {
    ...
    try {
      const response = await fetch("/api/generate", {
        ...
        body: JSON.stringify({
          ...
          contentType: "text",
          prompt: `REESCREVA ESTA LEGENDA DE FORMA DIFERENTE, MANTENDO O SENTIDO: "${data.finalPrompt}".`,
          fileContents: [],
        }),
      });
    ...
    <div className="flex justify-center w-full animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      <div className="w-full max-w-[470px] bg-black border border-zinc-800 rounded-xl overflow-hidden shadow-2xl flex flex-col">
        ...
        <div className="flex items-center justify-between p-3 border-b border-zinc-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-0.5">Original Audio</p>
            </div>
          </div>
        ...
        <div className="p-3 pb-6 bg-black relative z-20">
          ...
          {/* --- SECÇÃO DE COMENTÁRIOS (COM SCROLL PERSONALIZADO) --- */}
          <div className="mt-2 border-t border-zinc-900 pt-3">

            {/* Lista de Comentários com Scroll */}
            {comments.length > 0 && (
                <div
                    // CORREÇÃO: Adicionada classe 'custom-scrollbar' e 'pr-2' para espaçamento
                    className="flex flex-col gap-3 mb-3 animate-in fade-in custom-scrollbar overflow-y-auto max-h-[350px] pr-2"
                >
