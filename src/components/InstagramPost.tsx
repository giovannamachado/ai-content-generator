...
const InstagramPost = ({ imagePrompt, caption, hashtags }: InstagramPostProps) => {
  ...
  // Estados para controle da visibilidade das opções (Mobile Support)
  ...
  const handleRegenerateImage = () => {
    // Mobile UX: Esconde o menu de opções imediatamente para mostrar o spinner/nova imagem
  ...
  const handleRegenerateCaption = async () => {
    // Mobile UX: Esconde o menu de opções imediatamente para mostrar o spinner/nova legenda
  ...
  // HANDLER: Clique na imagem (Smart Toggle)
  const handleImageAreaClick = () => {
    // Detecta se o dispositivo tem suporte a hover (Desktop/Mouse)
  ...
  // HANDLER: Clique na legenda (Smart Toggle)
  const handleCaptionAreaClick = () => {
    const canHover = window.matchMedia("(hover: hover)").matches;
    if (canHover) return;
  ...
  return (
    <div className="flex justify-center w-full animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      <div className="w-full max-w-[470px] bg-black border border-zinc-800 rounded-xl overflow-hidden shadow-2xl flex flex-col">
        ...
        {/* Área da Imagem */}
        <div
            className="aspect-square w-full bg-zinc-900 relative group overflow-hidden cursor-pointer"
            onClick={handleImageAreaClick}
        >
            <img
                src={imageUrl}
                alt="Conteúdo Gerado"
                className={`w-full h-full object-cover transition-all duration-300 group-hover:scale-105 group-hover:blur-[2px] group-hover:grayscale-[0.7] ${isRegeneratingImage ? 'opacity-50' : 'opacity-100'} ${showImageOptions ? 'scale-105 blur-[2px] grayscale-[0.7]' : ''}`}
                loading="lazy"
            />
            ...
            <div
                className={`
                    absolute inset-0 bg-black/40 flex flex-col items-center justify-center z-10 backdrop-blur-[2px] transition-all duration-300
                    ${showImageOptions
                        ? 'opacity-100 pointer-events-auto' // Mobile/Active: Visível e Interativo
                        : 'opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto' // Desktop/Hover: Visível apenas no hover
                    }
                `}
            >
                <button
        ...
        <div className="p-3 pb-6 bg-black relative z-20">
          ...
          <div
            className="relative group rounded-xl -mx-2 px-2 py-2 transition-colors hover:bg-zinc-900/30 cursor-pointer"
            onClick={handleCaptionAreaClick}
          >
            <div className={`transition-all duration-300 ${isRegeneratingCaption ? 'opacity-50 blur-[1px]' : 'group-hover:opacity-40 group-hover:blur-[1px]'} ${showCaptionOptions && !isRegeneratingCaption ? 'opacity-40 blur-[1px]' : ''}`}>
